...
В предыдущих лекциях мы уже несколько раз выбирали “хороший” базис: для билинейных форм он убирал смешанные члены, в евклидовом пространстве ортонормированный базис упрощал длины и углы. Теперь тот же вопрос задаём для линейного оператора: можно ли выбрать базис так, чтобы матрица оператора стала диагональной?

Часть этого языка уже появлялась в лекции о Гамильтоне–Кэли и жордановой форме как необходимый минимум. Здесь мы возвращаемся к собственным значениям системно: отдельно разбираем характеристический многочлен, собственные подпространства, кратности и точные критерии диагонализируемости.

Главная линия лекции:
$$
\text{собственные направления}\to \text{собственные подпространства}\to \text{прямая сумма}\to \text{собственный базис}\to \text{диагонализация}.
$$
...
Это важная связь с лекциями о системах и ядрах: сначала значение $\lambda$ превращает оператор в новый оператор $A-\lambda I$, а затем собственные векторы ищутся как ненулевые элементы его ядра.
...
Поэтому собственное подпространство содержит не только отдельные “стрелки”, а всю линейную оболочку направлений, на которых оператор действует одинаково. Нуль лежит в $E_\lambda$, хотя собственным вектором не считается.
...
Это ровно та же прямая сумма из лекции о подпространствах: вектор из суммы собственных подпространств раскладывается по разным собственным значениям единственным образом.
...
Матрица $C$ здесь — это не произвольная замена базиса. Её столбцы должны быть собственными векторами, а диагональ $D$ должна содержать соответствующие им собственные значения в том же порядке.
...
Если этих равенств не хватает, появляются жордановы блоки: оператор всё ещё можно упростить выбором базиса, но уже не до диагонального вида. А следующий шаг курса — перейти к комплексным пространствам, где характеристические многочлены имеют больше корней, а эрмитова геометрия даёт особенно сильные версии спектральной теоремы.

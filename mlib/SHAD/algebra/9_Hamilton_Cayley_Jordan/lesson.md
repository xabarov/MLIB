...
<details>
<summary>Комментарий: не всякая нильпотентная матрица сразу имеет такой вид</summary>

Нет, матрица из примера — это лишь один частный случай: **один жорданов блок** для собственного значения $0$.

Вообще нильпотентная матрица определяется условием
$$
N^m=0
$$
для некоторого $m$, и в исходном базисе она совсем не обязана выглядеть как матрица с единицами прямо над диагональю.

Однако существует базис, в котором любая нильпотентная матрица приводится к блочно-диагональному виду, где каждый блок имеет форму
$$
J_k(0)=
\begin{pmatrix}
0 & 1 & 0 & \dots & 0\\
...
То есть верно не то, что **все** нильпотентные матрицы уже имеют такой вид, а то, что любую из них можно **привести** к такому виду заменой базиса.
...
\end{pmatrix}
$$
тоже нильпотентна, хотя над диагональю стоит не $1$, а $2$.
</details>

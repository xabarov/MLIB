...
Все файлы лежат в каталоге [`assets/`](assets/). Декоративные картинки задаются в `lecture_llm_images.json` и собираются скриптом `generate_images.py` из корня `mlib/`; графики и GIF — скриптом `generate_visuals.py` в этой же папке. Ниже рисунки встроены по смыслу разделов.
...
*Рис. 1. Общая иллюстрация к разделу (LLM, стиль SHAD / `lecture_llm_images.json`).*
...
*Рис. 2. Та же подстановка «верхний ряд $\to$ нижний ряд» (matplotlib).*
...
*Рис. 3. Число подстановок $|S_n| = n!$ растёт очень быстро (matplotlib).*
...
*Рис. 4. Наглядно: транспозиция меняет местами две метки (matplotlib + imageio).*
...
*Рис. 5–6. Цикл как ориентированный обход по вершинам (matplotlib / GIF).*
...
*Рис. 7. Визуальная метафора непересекающихся циклов (LLM, стиль SHAD).*
...
*Рис. 8. Разложение $\sigma = (1\ 3)(2\ 5)(4\ 6)$ (matplotlib).*

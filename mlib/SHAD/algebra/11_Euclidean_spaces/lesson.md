...
В прошлой лекции мы изучали симметрические билинейные формы и видели, что у них может быть разная сигнатура: положительные, отрицательные и нулевые направления. Евклидово пространство получается в самом “хорошем” случае, когда форма положительно определена.

Главная линия лекции:
$$
\text{положительно определённая форма}\to \text{скалярное произведение}\to \text{длины и углы}\to \text{проекции}\to \text{ортонормированные базисы}.
$$
...
Это ровно частный случай квадратичных форм из прошлой лекции с сигнатурой $(n,0,0)$. В отличие от неопределённой формы, здесь $\langle x,x\rangle$ всегда можно воспринимать как квадрат длины.

Для симметрической матрицы положительную определённость удобно проверять критерием Сильвестра: все главные угловые миноры должны быть положительны. Для матрицы $2\times 2$ это означает
$$
a_{11}>0,\qquad \det A>0.
$$
...
Такое разложение работает именно потому, что скалярное произведение положительно определено: у ненулевого вектора не может быть нулевой длины. Для произвольной симметрической формы из прошлой лекции аналогичные размерностные свойства могут ломаться при вырождении.
...
Поэтому ортонормированный базис — это базис, в котором евклидова геометрия выглядит как стандартная геометрия $\mathbb{R}^n$: координаты читаются скалярными произведениями, а матрица скалярного произведения равна единичной.
...
Смысл формулы: проекция — это часть $x$, которая лежит в $U$, а остаток — ошибка, перпендикулярная $U$. Поэтому проекция даёт ближайший к $x$ вектор из подпространства.
...
Это условие про совпадение оболочек важно: алгоритм не просто делает векторы перпендикулярными, а сохраняет уже построенное подпространство на каждом шаге. Поэтому после ортогонализации мы не теряем исходное пространство.
...
$$
\left\|\sum_{i=1}^k c_i v_i\right\|^2=\sum_{i,j=1}^k c_i c_j\langle v_i,v_j\rangle=c^T G c.
$$

Если $v_1,\dots,v_k$ линейно независимы, то сумма $\sum_i c_i v_i$ равна нулю только при $c=0$. Значит, квадратичная форма $c^T G c$ положительно определена, и $\det G>0$.

Если векторы зависимы, существует ненулевой $c$, для которого $\sum_i c_i v_i=0$. Тогда $c^T G c=0$, и матрица Грама вырождена.
...
Здесь снова видна связь с предыдущей лекцией: матрица Грама — это матрица скалярного произведения, ограниченного на систему векторов. Её определитель показывает, есть ли у этих векторов ненулевой объём.
...
Дальше эта идея будет полезна при работе с собственными векторами: для симметрических операторов в евклидовом пространстве ортогональность и выбор хорошего базиса становятся главным инструментом упрощения матрицы.

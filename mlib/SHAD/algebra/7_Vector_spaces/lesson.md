...
В прошлой лекции матрицы были удобным языком для систем, обратимости и композиции действий. Теперь мы отделяем саму идею от конкретной таблицы чисел: векторное пространство — это место, где можно складывать и растягивать объекты, базис превращает эти объекты в координаты, а подпространства описывают “линейные куски” внутри пространства.

Главная линия лекции:
$$
\text{пространство}\to \text{базис}\to \text{координаты}\to \text{подпространства}\to \text{сумма и прямая сумма}.
$$

Как читать эту лекцию:

- разделы 1--4 дают язык пространств, оболочек, независимости и базиса;
- разделы 5--11 объясняют координаты и смену базиса;
- разделы 12--13 связывают подпространства с однородными системами;
- разделы 14--25 разбирают сумму, пересечение и прямую сумму подпространств;
- разделы 26--29 собирают экзаменационный минимум, ошибки и самопроверку.

![Иллюстрация к теме векторных пространств: линейные комбинации, координаты и подпространства](assets/hero_vector_spaces.png)
...
На иллюстрации ниже линейная оболочка показана как минимальное подпространство, содержащее выбранные векторы.

![Линейная оболочка как минимальное подпространство](assets/span_subspace_editorial.png)
...
Схема ниже показывает идею: базис одновременно порождает подпространство и не содержит лишних зависимых направлений.

![Базис и размерность подпространства](assets/basis_dimension_editorial.png)
...
Иными словами, матрица $S$ переводит координаты в базисе $e$ в обычную запись вектора, а $S^{-1}$ делает обратное. Здесь снова используется идея из прошлой лекции: обратимость матрицы означает, что столбцы действительно задают базис и координаты восстанавливаются единственным образом.
...
Здесь легко ошибиться в направлении. Индекс
$$
e\leftarrow f
$$
читается так: “координаты из базиса $f$ переводятся в координаты базиса $e$”. Поэтому слева в формуле стоит $[v]_e$, а справа — $[v]_f$.

| Что известно | Что умножаем | Что получаем |
|---|---|---|
| координаты в базисе $f$ | $C_{e\leftarrow f}[v]_f$ | координаты того же вектора в базисе $e$ |
| координаты в базисе $e$ | $C_{e\leftarrow f}^{-1}[v]_e$ | координаты того же вектора в базисе $f$ |
...
Важно: сумма подпространств — это не объединение множеств $U\cup W$. В сумму попадают все векторы, которые можно собрать как “кусок из $U$ плюс кусок из $W$”. Поэтому даже если $U$ и $W$ сами маленькие, их сумма может оказаться гораздо больше.
...
Рабочая интуиция такая же, как при подсчёте элементов двух пересекающихся множеств, только вместо количества элементов мы считаем количество независимых направлений. Если общая часть имеет размерность $k$, то эти $k$ направлений были учтены и в $U$, и в $W$.
...
Для двух подпространств это условие особенно наглядно: у них нет общего ненулевого направления. Значит, ненулевой вектор из $U$ не может быть полностью компенсирован ненулевым вектором из $W$.
...
Здесь слово “прямая” означает не геометрическую прямую, а отсутствие скрытого пересечения. Если пересечение нетривиально, один и тот же общий вектор можно переложить из слагаемого $U$ в слагаемое $W$, и разложение перестаёт быть единственным.
...
В следующей лекции мы добавим к этой картине функции между пространствами. Линейное отображение — это правило, которое сохраняет линейные комбинации, а его матрица появляется потому, что мы уже умеем выбирать базисы и записывать векторы координатами.

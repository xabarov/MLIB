...
![Иллюстрация к теме условной вероятности: сужение пространства исходов при наступлении условия](assets/hero_conditional_probability.png)
...
### Пример: независимость ≠ несовместность

Бросаем кубик. $A = \{\text{чётное}\} = \{2,4,6\}$, $B = \{1,2,3,4\}$.

- $\mathbb{P}(A) = 1/2$, $\mathbb{P}(B) = 2/3$, $\mathbb{P}(A \cap B) = |\{2,4\}|/6 = 1/3$.
- $\mathbb{P}(A)\cdot\mathbb{P}(B) = 1/2 \cdot 2/3 = 1/3 = \mathbb{P}(A \cap B)$ → **независимы**.
- $A \cap B = \{2,4\} \ne \emptyset$ → **не несовместны**.

Вывод: независимые события могут пересекаться. Несовместные непустые события всегда **зависимы** (знание $A$ исключает $B$):
$$\mathbb{P}(A \cap B) = 0 \ne \mathbb{P}(A)\mathbb{P}(B) \text{ (если оба имеют ненулевую вер-ть)}.$$

...
Представьте **таблицу**: строки — элементарные исходы $\omega_1, \omega_2, \ldots$ (сценарии), столбцы — моменты времени $n = 1, 2, 3, \ldots$ В каждой клетке — значение $X_n(\omega_i)$.
...
- **Сходимость п.н.** — почти каждая **строка** сходится к $X(\omega_i)$.
- **Сходимость по вероятности** — в каждом **столбце** всё меньше строк с большим отклонением.
- **Сходимость в $L^p$** — среднее $p$-й степени отклонений по **столбцу** → 0.
- **Сходимость по распределению** — **гистограммы столбцов** становятся похожи на гистограмму $X$.

Ключевое: п.н. — это про поведение отдельных **траекторий**. Остальные три — про поведение **среза в момент** $n$.

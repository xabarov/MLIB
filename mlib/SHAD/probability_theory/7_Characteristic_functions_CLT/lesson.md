...
### Пример: вычисление х.ф. из определения

$X \sim \mathrm{Ber}(p)$: $\mathbb{P}(X=0)=1-p$, $\mathbb{P}(X=1)=p$.

$$\varphi_X(t) = (1-p)e^{it\cdot0} + p\,e^{it\cdot1} = (1-p) + pe^{it}.$$

Проверим свойства: $\varphi_X(0) = (1-p)+p = 1$ ✓; $|\varphi_X(t)| = |1-p+pe^{it}| \le 1$ ✓.

Производная: $\varphi_X'(t) = ipe^{it}$, поэтому $\mathbb{E}[X] = \varphi_X'(0)/i = ip/i = p$ ✓.
...
### Пример: применение свойства независимости

$X_1, \ldots, X_n$ — н.о.р. $\mathrm{Poisson}(\lambda)$. Найти х.ф. суммы $S_n = X_1 + \cdots + X_n$.

По свойству 5: $\varphi_{S_n}(t) = [\varphi_{X_1}(t)]^n = [\exp(\lambda(e^{it}-1))]^n = \exp(n\lambda(e^{it}-1))$.

Это х.ф. $\mathrm{Poisson}(n\lambda)$ — что согласуется с аддитивностью Пуассона.

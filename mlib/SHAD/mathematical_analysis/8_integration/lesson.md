...
### Почему появляется якобиан

В одной переменной при замене переменной появляется множитель вида $dx=\varphi'(u)\,du$. В двух переменных происходит то же самое, только вместо длины меняется уже **площадь**.

Если в плоскости $(u,v)$ взять маленький прямоугольник со сторонами $\Delta u$ и $\Delta v$, то после преобразования
$$
x=x(u,v),\qquad y=y(u,v)
$$
он обычно переходит не в прямоугольник, а в маленький параллелограмм.

Площадь этого параллелограмма приближённо равна
$$
|J|\,\Delta u\,\Delta v.
$$

Именно поэтому в формуле замены переменных появляется множитель
$$
\left|\frac{\partial(x,y)}{\partial(u,v)}\right|.
$$

Здесь важен именно модуль, потому что площадь должна быть неотрицательной. Если якобиан отрицателен, это означает смену ориентации, но не отрицательную площадь.

### Как делать замену переменных на практике

Обычно удобно идти по такому алгоритму:

1. ввести новые переменные $u,v$ или, наоборот, выразить $x,y$ через $u,v$;
2. переписать область интегрирования в новых переменных;
3. вычислить якобиан;
4. заменить подынтегральную функцию;
5. не забыть заменить элемент площади: $dx\,dy \to |J|\,du\,dv$.
...
### Ещё один пример: площадь эллипса
...
Естественная замена здесь такая:
$$
x=au,\qquad y=bv.
$$
...
Но площадь единичного круга равна $\pi$, поэтому
$$
\iint\limits_{\frac{x^2}{a^2}+\frac{y^2}{b^2}\le 1} 1\,dx\,dy = \pi ab.
$$

Это хороший пример, потому что здесь очень наглядно видно: преобразование просто растягивает круг в $a$ раз по оси $x$ и в $b$ раз по оси $y$, поэтому площадь умножается на $ab$.

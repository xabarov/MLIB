...
![Функция $f(x)=x^4-4x^3$ с её первой и второй производной: на $[0,3]$ функция монотонно убывает, поэтому между точками перегиба $x=0$ и $x=2$ максимум не возникает](assets/x4_minus_4x3_analysis.png)

На картинке видно главное: точки перегиба и экстремумы — про **разные** вещи. Точка перегиба — это смена выпуклости (знак $f''$), а локальный максимум/минимум — смена монотонности (знак $f'$). На $(0,3)$ имеем $f'(x)=4x^2(x-3)<0$, поэтому $f$ строго убывает на всём этом интервале — у убывающей функции внутри интервала локальный максимум невозможен. Между $x=0$ и $x=2$ при этом меняется выпуклость (с вверх на вниз), но это никак не противоречит убыванию: график «изгибается», оставаясь спускающимся. Только в $x=3$, где $f'$ меняет знак, появляется локальный минимум.

...
**Определение.** *Система непересекающихся множеств* (СНМ, Union-Find, DSU) — структура данных, которая поддерживает разбиение конечного множества $\{1, \ldots, n\}$ на попарно непересекающиеся подмножества и выполняет операции:
...
**Почему ранг ограничивает высоту.** Ключевое наблюдение: дерево с корнем ранга $r$ содержит не менее $2^r$ элементов. Индукция по $r$: база — корень ранга 0 является одиночным элементом ($2^0 = 1$); переход — ранг корня увеличивается до $r$ только в одном случае: при слиянии *двух* корней ранга $r-1$, каждый из которых по предположению держит $\ge 2^{r-1}$ элементов, итого $\ge 2^r$ (при слиянии разных рангов ранг вообще не растёт). Отсюда $2^r \le n$, то есть любой ранг не превышает $\log_2 n$; а высота дерева не превышает ранга корня (при подвешивании меньшего под большего высота либо не меняется, либо растёт вместе с рангом). Значит, путь от любого элемента до корня короче $\log_2 n$ шагов — это и есть оценка Find. Заодно видно, почему вырождение из раздела 2 больше невозможно: цепочка $0 \to 1 \to 2 \to 3$ высоты 3 потребовала бы ранга 3, то есть минимум $2^3 = 8$ элементов, а не 4.
...
**Откуда берётся «почти константа» (интуиция, без строгого доказательства).** Дорогой Find сам себя окупает: пройдя по цепочке из $k$ узлов, он платит $O(k)$, но при этом уничтожает эту цепочку — все $k$ узлов перевешиваются прямо под корень, и следующие запросы к ним стоят $O(1)$. Чтобы Find снова стал дорогим, кто-то должен заново построить длинный путь, а объединение по рангу делает это чрезвычайно трудным. Формальный стержень рассуждения — монотонность: ранги вдоль пути к корню строго возрастают, и при каждом перевешивании узел получает родителя *строго большего* ранга, чем прежний; ранг родителя узла может только расти. Если сгруппировать ранги в стремительно растущие блоки $\{1\}, \{2\}, \{3, 4\}, \{5, \ldots, 2^4\}, \{17, \ldots, 2^{16}\}, \ldots$, то узел пересекает границу блока лишь несколько раз за всю жизнь структуры, а внутри блока каждый шаг Find продвигает его к «границе». Число блоков до $n$ — это и есть $\alpha(n)$-подобная величина (в таком грубом разбиении $O(\log^* n)$: для $n = 2^{65536}$ всего 5 блоков); аккуратный учёт того же аргумента даёт $O(\alpha(n))$. Мораль: ранги не дают деревьям вытягиваться, а сжатие пути необратимо уплощает всё, к чему прикоснулось, — вместе они не оставляют Find ни одного шанса работать долго дважды.
...
![СНМ: дерево до и после сжатия пути](assets/dsu_compression.png)

На рисунке эта трассировка (с двумя дополнительными элементами 4 и 5, уже висящими под корнем): слева — цепочка $0 \to 1 \to 2 \to 3$, по которой $\texttt{find}(0)$ вынужден пройти целиком; справа — состояние после сжатия: все пройденные узлы перевешены непосредственно под корень 3. Следующий вызов $\texttt{find}$ для любого из них дойдёт до корня за один шаг. Именно это «выпрямление» деревьев и даёт амортизированную оценку $O(\alpha(n))$.
...
![Префикс-функция строки ABABCABAB: таблица и совпадение префикса с суффиксом](assets/prefix_function.png)

На рисунке — та же строка с массивом $\pi$ под ней. Внизу показано, откуда берётся $\pi[8] = 4$: префикс $\texttt{ABAB}$ (синий) совпадает с суффиксом $\texttt{ABAB}$ строки $s[0..8]$ (оранжевый). Обратите внимание на позицию $i=4$: символ $\texttt{C}$ «обнуляет» префикс-функцию, потому что ни один префикс строки не заканчивается на $\texttt{C}$.

**Алгоритм вычисления за $O(n)$:**

Ключевое наблюдение: $\pi[i] \le \pi[i-1] + 1$, причём равенство достигается ровно тогда, когда $s[i] = s[\pi[i-1]]$ — новый символ «продолжает» лучший префикс-суффикс предыдущей позиции. Если же $s[i] \ne s[k]$ при $k = \pi[i-1]$, следующий кандидат на длину — не $k - 1$, а $\pi[k-1]$: это длина наибольшего префикса, который всё ещё остаётся суффиксом уже совпавшего куска. Мы «откатываемся» по цепочке $k,\ \pi[k-1],\ \pi[\pi[k-1]-1], \ldots$ до первого совпадения или до нуля.

Разберём один шаг на строке $s = \texttt{ABABCABAB}$: пусть считаем $\pi[4]$ (символ $s[4] = \texttt{C}$). Кандидат $k = \pi[3] = 2$, сравниваем $s[4] = \texttt{C}$ с $s[2] = \texttt{A}$ — не совпало; откат $k = \pi[1] = 0$, сравниваем $s[4]$ с $s[0] = \texttt{A}$ — снова нет; $k = 0$, значит $\pi[4] = 0$.
...
![КМП: образец ABABC в тексте ABABABABC, откат по префикс-функции](assets/kmp_search.png)

На рисунке та же трассировка: сверху текст $T$ с номерами позиций, под ним образец $P$ в первом выравнивании. Числа $j$ над текстом — значение счётчика совпадения после обработки соответствующего символа. В позиции 4 происходит несовпадение $\texttt{A} \ne \texttt{C}$ (помечено ≠), и оранжевая дуга показывает главный трюк КМП: вместо отката в тексте образец «сдвигается» прыжком $j = \pi[3] = 2$ — уже совпавший кусок $\texttt{AB}$ не перечитывается. Зелёная пометка у позиции 8 — момент полного совпадения ($j = 5$).

### Почему откат j = π[j−1] не пропускает вхождений

**Ключевое наблюдение.** Пусть после чтения $T[0..i]$ длина текущего совпадения равна $j$: последние $j$ символов текста — это префикс $P[0..j-1]$ образца. Рассмотрим любое вхождение $P$, начинающееся внутри этого окна: его уже прочитанная часть длины $k < j$ обязана совпадать с текстом, то есть быть *суффиксом* куска $P[0..j-1]$, и одновременно она — *префикс* $P$. Значит, возможные вхождения — это в точности те $k$, для которых префикс $P$ длины $k$ равен суффиксу $P[0..j-1]$ той же длины. А множество всех таких $k$ — ровно цепочка $\pi[j-1] > \pi[\pi[j-1]-1] > \cdots > 0$: наибольший префикс-суффикс даёт $\pi[j-1]$, каждый более короткий является префикс-суффиксом наибольшего, и индукцией вся цепочка перечисляет их без пропусков.

**Инвариант.** После обработки символа $T[i]$ значение $j$ равно длине *наибольшего* префикса $P$, являющегося суффиксом $T[0..i]$. База: до первого символа $j = 0$ — пустой префикс. Переход: при несовпадении $T[i] \ne P[j]$ цикл `while` спускается по цепочке $j \to \pi[j-1] \to \cdots$, перебирая всех кандидатов **в порядке убывания**, и останавливается на первом, которого удаётся продолжить символом $T[i]$, — то есть на наибольшем возможном. Длины $k$ с $\pi[j-1] < k < j$, «перепрыгнутые» одним присваиванием, кандидатами не являются: вхождение с таким сдвигом разошлось бы с текстом ещё в *уже прочитанных* символах — проверять его по тексту не нужно, отказ доказан сравнением образца с самим собой при подсчёте $\pi$.

Из инварианта следует полнота поиска: если вхождение $P$ оканчивается в позиции $i$, то весь $P$ (длины $n$) — суффикс $T[0..i]$, значит, в этот момент $j = n$ и алгоритм обязательно сообщает о вхождении. Завершаемость и линейность — из уже знакомой амортизации: $j$ увеличивается не более чем на 1 за символ текста, поэтому суммарно все откаты стоят $O(|T|)$, плюс $O(|P|)$ на подсчёт $\pi$.

В числах трассировки: в позиции $i = 4$ имеем $j = 4$, совпавший кусок — $\texttt{ABAB}$. Его префикс-суффиксные длины: $\pi[3] = 2$ ($\texttt{AB}$), далее $\pi[1] = 0$. Прыжок $4 \to 2$ перепрыгивает $k = 3$: префикс $\texttt{ABA}$ не равен суффиксу $\texttt{BAB}$, вхождение со сдвигом на 1 невозможно — и алгоритм «знает» это заранее, не глядя в текст. Кандидат $k = 2$ продолжается: $P[2] = \texttt{A} = T[4]$, и $j$ становится 3.

![Почему прыжок по префикс-функции ничего не пропускает](assets/kmp_no_skip.png)

На рисунке — все кандидаты на вхождение внутри совпавшего окна $\texttt{ABAB}$. Для каждого сдвига показано сравнение начала образца с концом окна: сдвиги с перекрытием $k = 3$ и $k = 1$ отвергаются уже на совпавшей части (красный крест — расхождение с *уже прочитанными* символами текста), и лишь перекрытие $k = 2 = \pi[3]$ выживает — именно к нему и прыгает алгоритм. Проверять промежуточные сдвиги по тексту бессмысленно: их судьба решена сравнением образца с самим собой.
...
- Узлы, соответствующие концам слов, помечены как *терминальные*.
...
![Бор для слов car, cart, cat, dog, dot](assets/trie_example.png)

На рисунке этот же бор в виде дерева: рёбра помечены символами, двойные оранжевые кружки — терминальные узлы (концы слов). Хорошо видна главная экономия бора: общий префикс `ca` слов `car`, `cart`, `cat` хранится один раз — три слова делят одну цепочку узлов и расходятся только на последнем символе. Заметьте также, что терминальный узел не обязан быть листом: узел `r` (конец слова `car`) имеет ребёнка `t`, продолжающего слово `cart`.

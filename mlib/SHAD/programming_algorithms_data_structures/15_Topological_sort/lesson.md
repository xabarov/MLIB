...
**Пошаговый разбор алгоритма Кана** на том же примере:

| Шаг | Очередь | Вершина | Действие | in-degree после |
|---|---|---|---|---|
| Начало | {0} | — | Только 0 имеет in-degree=0 | [0,1,1,2,1,1] |
| 1 | {1,2} | 0 | Берём 0, убавляем in-deg[1] и in-deg[2] | [0,0,0,2,1,1] |
| 2 | {2,1} | 1 | Берём 1, убавляем in-deg[3] | [0,0,0,1,1,1] |
| 3 | {1,4} | 2 | Берём 2, убавляем in-deg[3],in-deg[4] | [0,0,0,0,0,1] |
| 4 | {4,3} | 1 | (порядок зависит от реализации) | ... |

Итоговый результат: `[0, 1, 2, 3, 4, 5]` или другой корректный порядок.

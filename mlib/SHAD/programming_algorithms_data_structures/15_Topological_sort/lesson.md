...
**Ключевое наблюдение.** `in_degree[v]` — это счётчик **невыведенных предшественников** вершины $v$. Вершину безопасно ставить в порядок ровно в тот момент, когда её счётчик обнулился: все, кто обязан стоять раньше неё, уже стоят раньше.

**Инвариант.** В любой момент работы алгоритма `in_degree[v]` равен числу рёбер $u \to v$, у которых $u$ ещё **не** попал в `result`. База: перед стартом `result` пуст, счётчики равны настоящим входящим степеням. Переход: вывод вершины $u$ сопровождается уменьшением счётчиков всех её out-соседей — ровно тех рёбер, которые перестали быть «невыведенными». Другие счётчики не меняются — инвариант сохранён.

**Корректность порядка.** Вершина $v$ попадает в очередь только при `in_degree[v] == 0`, то есть — по инварианту — когда **все** её предшественники уже выведены. Значит, для любого ребра $u \to v$ вершина $u$ оказывается в `result` строго раньше $v$, а это и есть определение топологического порядка. В трассировке выше это видно на вершине 3: на шаге 2 она осталась ждать со счётчиком 1 и вошла в очередь лишь на шаге 3, когда оба предшественника (1 и 2) попали в результат.

**Почему ненулевой остаток = цикл.** Пусть алгоритм остановился, а множество невыведенных вершин $S$ непусто. У каждой $v \in S$ счётчик положителен (иначе она попала бы в очередь в момент обнуления), то есть по инварианту у неё есть предшественник **из $S$**. Пойдём из любой вершины $S$ назад по предшественникам: $v_0 \leftarrow v_1 \leftarrow v_2 \leftarrow \cdots$ — каждый шаг остаётся внутри конечного множества $S$, поэтому не позднее чем через $|S|$ шагов какая-то вершина повторится — найден направленный цикл, целиком лежащий в $S$. Обратно: если граф — DAG, тот же аргумент показывает, что в любом непустом $S$ есть вершина без предшественников в $S$, поэтому очередь не опустеет преждевременно и алгоритм выведет все $n$ вершин. Итого проверка `result.size() < n` — точный критерий наличия цикла, а не эвристика.

**Сложность из структуры.** Каждая вершина входит в очередь не более одного раза (её счётчик обнуляется однократно), каждое ребро $u \to v$ уменьшает счётчик ровно один раз — при выводе $u$. Вместе с начальным подсчётом степеней это $O(V + E)$.

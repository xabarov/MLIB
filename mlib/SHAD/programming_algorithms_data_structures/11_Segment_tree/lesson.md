...
**Почему ответ верен: query собирает точное покрытие.** **Ключевое наблюдение:** отрезки узлов образуют иерархию — каждый узел разбивается ровно на два *непересекающихся* отрезка потомков, поэтому узлы, на которых query останавливается по случаю 2, автоматически не пересекаются и в объединении дают в точности $[ql, qr]$. Формально докажем индукцией по дереву инвариант: *вызов query от узла с отрезком $[l, r]$ возвращает ровно $\sum_{i \in [l,r] \cap [ql,qr]} A[i]$*. База — случаи 1 и 2: при пустом пересечении сумма пуста, и возвращается нейтральный 0; при $[l,r] \subseteq [ql,qr]$ пересечение равно всему $[l,r]$, а `tree[node]` хранит именно эту сумму — это инвариант из раздела 2, который build устанавливает, а update поддерживает пересчётом узлов по пути к листу. Переход — случай 3: множество $[l,r] \cap [ql,qr]$ распадается на две непересекающиеся части, $[l,\text{mid}] \cap [ql,qr]$ и $[\text{mid}{+}1,r] \cap [ql,qr]$; по предположению индукции рекурсивные вызовы возвращают ровно их суммы, значит каждый индекс запроса учитывается один раз — ничего не теряется и не удваивается. Завершаемость очевидна: отрезок на каждом шаге строго укорачивается, глубина рекурсии не превышает высоты дерева. В трассировке ниже это видно живьём: запрос $[2,4]$ распался на непересекающееся покрытие $[2,2] \sqcup [3,4]$, и $5 + 16 = 21$.

**Сложность:** $O(\log n)$ — на каждом уровне обрабатывается не более 4 узлов (доказательство — после трассировки).
...
**Почему именно $O(\log n)$?** Рекурсия продолжается только из «частичных» (синих) узлов — серые и оранжевые возвращают ответ мгновенно. **Ключевое наблюдение:** на каждом уровне дерева частичных узлов не больше двух. Пусть узел с отрезком $[l, r]$ частичный: он пересекает $[ql, qr]$, но не лежит в нём целиком. Раз отрезок непрерывен, он обязан «переступать» через границу запроса: либо $l < ql \le r$ — тогда $[l,r]$ содержит соседнюю пару индексов $(ql{-}1,\, ql)$, то есть накрывает левую границу, либо $l \le qr < r$ — тогда накрывает правую пару $(qr,\, qr{+}1)$. Но отрезки узлов одного уровня попарно не пересекаются, поэтому пару $(ql{-}1, ql)$ содержит максимум один узел уровня, и пару $(qr, qr{+}1)$ — максимум один. Итого $\le 2$ частичных узла на уровень. Все остальные посещённые узлы уровня — дети частичных узлов предыдущего уровня, их $\le 4$, и каждый отрабатывает за $O(1)$. Уровней $\lceil \log_2 n \rceil + 1$, значит суммарно $O(\log n)$. Тот же аргумент дословно переносится на `range_update` с ленивой пропагацией из раздела 6: там рекурсивный спуск тоже идёт только из частичных узлов.

![Не более двух частичных узлов на уровень: query [3,12] при n = 16](assets/two_partials.png)

Диаграмма получена честной симуляцией запроса $[3, 12]$ на дереве из 16 листьев — случай «неудобных» границ, когда запрос режет оба поддерева. Синие узлы — частичные (рекурсия идёт дальше), оранжевые — целиком внутри (готовый агрегат), серые — вне запроса; числа справа показывают количество частичных узлов на каждом уровне. Хорошо видно, как работает доказательство: левую границу (пару индексов 2–3) на каждом уровне накрывает не больше одного узла, правую (пару 12–13) — тоже, поэтому «синяя зона» не разрастается: 1, 2, 2, 2, 0 — а вовсе не удваивается, как могло бы показаться по картинке бинарного дерева.
...
**Почему это корректно.** Сформулируем инвариант ленивого дерева: *для каждого узла истинная сумма его отрезка равна `tree[node]` плюс сумма тегов всех его строгих предков, умноженная на длину отрезка* (свой тег узел уже учёл: в момент постановки тега мы одновременно исправили `tree[node]`). Обе операции сохраняют инвариант. `range_update` на полностью покрытом узле прибавляет к `tree[node]` ровно $v \cdot (r - l + 1)$ и записывает $v$ в тег — для самого узла сумма стала честной, а для каждого потомка «долг предков» вырос ровно на ту величину, на которую выросла его истинная сумма. `push_down` переносит долг на уровень вниз, не меняя ни одной истинной суммы: `tree` потомка увеличивается на $\text{lazy} \times \text{длина}$, тот же долг дописывается в его тег, у родителя тег обнуляется — баланс «значение + долги предков» у всех узлов поддерева не изменился. Читает же `range_query` значение `tree[node]` только после того, как `push_down` выполнен во всех узлах на пути от корня, — то есть в момент чтения долгов над узлом нет, и по инварианту `tree[node]` равно истинной сумме. На средней панели диаграммы это видно: листья $[2,2]$ и $[3,3]$ хранят устаревшие 3 и 4, но их истинные значения $3 + 5$ и $4 + 5$ восстанавливаются по тегу родителя, а сумма в корне (25) уже честная.

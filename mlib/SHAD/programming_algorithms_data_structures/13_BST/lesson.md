...
### Почему удаление с двумя потомками корректно

**Ключевое наблюдение.** На месте удаляемого узла $v$ имеет право стоять только ключ, который больше **всех** ключей левого поддерева $v$ и меньше **всех** ключей правого. В отсортированном inorder-порядке таких кандидатов ровно два — непосредственные соседи ключа $v$: максимум левого поддерева (предшественник) и минимум правого (преемник). Алгоритм берёт преемника $s = \min(\text{right}(v))$.

**Инвариант (свойство BST) сохраняется.** Проверим оба неравенства для $s$ на месте $v$:
- $s$ лежит в правом поддереве $v$, значит $s$ больше любого ключа левого поддерева $v$;
- $s$ — минимум правого поддерева, значит строго меньше всех остальных его ключей (ключи различны).

Больше нигде дерево не менялось: у остальных узлов поддеревья те же, лишь из правого поддерева $v$ исчез один ключ. Проверка через inorder на примере выше: до удаления — $1, 3, 4, 5, 6, 7, 8$; после замены $3 \to 4$ и удаления старого узла 4 — $1, 4, 5, 6, 7, 8$. Последовательность осталась возрастающей — просто без тройки, а это и есть определение корректного удаления.

**Завершаемость.** Рекурсивный вызов `remove(root->right, successor->key)` не может снова попасть в случай двух потомков: у $s$ нет левого потомка (иначе $s$ не был бы минимумом), поэтому вложенное удаление завершается случаем 1 или 2 — одним перевешиванием указателя. Каскада подмен не возникает: на одно удаление — не более одной замены ключа.

**Сложность.** Спуск от корня к $v$ и спуск от $v$ к преемнику $s$ вместе образуют один нисходящий путь длиной не более $h$; повторный поиск `successor->key` идёт по этому же пути. Итого $O(h)$: в примере — 5 → 3 (поиск удаляемого), затем 3 → 4 (минимум правого поддерева).
...
**Интуиция.** Ротация — это операция за $O(1)$: переставляются всего три указателя. Ключевое наблюдение: ротация **не меняет inorder-порядок** узлов, а значит, сохраняет свойство BST. Меняются только высоты — тяжёлое поддерево «поднимается» на уровень выше, лёгкое «опускается».
...
![Правое вращение вокруг y: до и после](assets/avl_rotation.png)

На схеме — правое вращение вокруг узла $y$ с перевесом слева ($bf(y) = +2$). Треугольники $A$, $B$, $C$ — целые поддеревья, их внутренняя структура не важна. После вращения $x$ становится новым корнем, $y$ — его правым потомком, а поддерево $B$ (в котором все ключи лежат между $x$ и $y$) «переезжает»: было правым поддеревом $x$, стало левым поддеревом $y$. Проверьте по картинке, что inorder-порядок $A < x < B < y < C$ одинаков до и после — именно поэтому свойство BST не нарушается.
...
**Почему select работает.** Инвариант рекурсии: при каждом вызове $\text{Select}(v, k)$ искомый элемент — это $k$-й наименьший **в текущем поддереве** $v$, причём $1 \le k \le \text{sz}(v)$. В корне это верно по постановке задачи. Переход сохраняет инвариант, потому что левое поддерево содержит ровно $\text{sz}(v.\text{left})$ наименьших ключей поддерева: при $k \le \text{sz}(v.\text{left})$ ответ целиком слева и остаётся там $k$-м; при $k > \text{sz}(v.\text{left}) + 1$ мы отбрасываем левое поддерево вместе с самим $v$ — ровно $\text{sz}(v.\text{left}) + 1$ наименьших элементов, поэтому номер уменьшается именно на эту величину. В трассировке выше это шаг в узле 3: отброшены $\{1, 3\}$, и «3-й во всём дереве» превратился в «1-го в поддереве узла 4» ($k = 3 - 2 = 1$). Каждый шаг спускается на уровень ниже — алгоритм завершается не более чем за $h$ шагов, а в момент остановки $k = \text{sz}(v.\text{left}) + 1$ инвариант гарантирует, что $v$ — именно $k$-й наименьший.
...
**Ошибка 1: проверять только прямых потомков вместо всего поддерева.**
Классический пример: корень 10, левый потомок 8, правый потомок 12, а у узла 12 левый потомок 6. Локально всё выглядит корректно ($6 < 12$), но глобально свойство BST нарушено: узел 6 лежит в **правом** поддереве корня 10, а $6 < 10$. Проверка «каждый узел больше левого потомка и меньше правого» — недостаточна. Правильная проверка: рекурсивно передавать допустимый интервал $(\min, \max)$ для каждого поддерева и требовать $\min < \text{key} < \max$.

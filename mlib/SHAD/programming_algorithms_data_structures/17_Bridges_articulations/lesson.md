...
- Конечный из ребра моста $(u, v)$ — точка сочленения (но не всегда оба его конца).
...
**2-SAT и SCC.** В задачах на 2-SAT граф импликаций анализируется на SCC, но обнаружение мостов в компоненте полезно для поиска «жёсткий» переменных — тех, чьё значение однозначно определено.

**Incremental connectivity (динамическая связность).** Bridge tree (дерево мостов) Тарьяна позволяет эффективно отвечать на запросы «связаны ли вершины $u$ и $v$?» при добавлении рёбер за $O(\alpha(n))$ амортизированно.

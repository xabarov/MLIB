...
Важная деталь: релаксации внутри одной итерации *каскадируют* — обновлённое значение сразу используется следующими рёбрами того же прохода. Проследим итерацию 1 по шагам:

- $(0,1,6)$: $\text{dist}[1] = 0 + 6 = 6$;
- $(0,2,7)$: $\text{dist}[2] = 7$;
- $(1,3,5)$: $\text{dist}[3] = 6 + 5 = 11$;
- $(1,4,-4)$: $\text{dist}[4] = 6 - 4 = 2$;
- $(2,3,-3)$: $\text{dist}[3] = \min(11,\ 7 - 3) = 4$ — уже используем свежее $\text{dist}[2] = 7$;
- $(3,1,-2)$: $\text{dist}[1] = \min(6,\ 4 - 2) = 2$ — и свежее $\text{dist}[3] = 4$.
...
На итерации 2 срабатывает лишь одно ребро: $(1,4,-4)$ даёт $\text{dist}[4] = 2 + (-4) = -2$ (обновлённое $\text{dist}[1] = 2$ появилось *после* того, как ребро $(1,4)$ было обработано на итерации 1). На итерации 3 изменений нет — срабатывает ранняя остановка, хотя гарантия была на $V - 1 = 4$ итерации.

Число «содержательных» итераций зависит от порядка перебора рёбер: если рёбра кратчайшего пути идут в списке «от конца к началу», за одну итерацию путь удлиняется лишь на одно ребро, и понадобятся все $V-1$ итераций. Оценка $V-1$ — это гарантия для худшего порядка.

![Форд-Беллман: таблица dist по итерациям](assets/bellman_ford_table.png)

На рисунке та же трассировка в виде таблицы: оранжевым выделены ячейки, изменившиеся по сравнению с предыдущей итерацией. Хорошо видно, как почти вся работа происходит на первой итерации (каскад релаксаций), итерация 2 «дочищает» одно значение, а итерация 3 — холостая: она лишь подтверждает, что алгоритм сошёлся.

### Почему хватает V − 1 итераций

**Ключевое наблюдение.** У кратчайшего пути оптимальны все его префиксы: если $s \to \cdots \to u \to v$ — кратчайший путь до $v$, то его начальный кусок $s \to \cdots \to u$ — кратчайший путь до $u$ (иначе, заменив кусок более коротким, мы укоротили бы и весь путь). Значит, кратчайший путь «собирается» по одному ребру за раз — и каждая итерация Форда-Беллмана гарантированно достраивает хотя бы одно очередное его ребро.

Точный **инвариант**: после $k$-й итерации для каждой вершины $v$

$$
\text{dist}[v] \;\le\; \text{длина кратчайшего пути из } s \text{ в } v \text{, использующего не более } k \text{ рёбер}.
$$

При этом всегда $\text{dist}[v] \ge \delta(s, v)$: релаксация присваивает `dist[v]` длину некоторого *реального* пути, короче кратчайшего она быть не может.

**Индукция по $k$.** База: до первой итерации $\text{dist}[s] = 0$ — кратчайший путь из 0 рёбер учтён. Переход: пусть кратчайший путь до $v$ из $\le k$ рёбер оканчивается ребром $(u, v)$. Его префикс до $u$ — кратчайший путь из $\le k-1$ рёбер, и по предположению индукции к началу $k$-й итерации $\text{dist}[u]$ уже не больше его длины. На $k$-й итерации алгоритм расслабляет **все** рёбра, в том числе $(u, v)$, поэтому после неё $\text{dist}[v] \le \text{dist}[u] + w(u,v)$ — не больше длины всего пути. Без отрицательных циклов кратчайший путь можно выбрать простым, то есть из $\le V-1$ рёбер, — после $V-1$ итераций оба неравенства смыкаются: $\text{dist}[v] = \delta(s, v)$. Завершаемость очевидна — число итераций фиксировано заранее; отсюда же сложность $O(VE)$: $V-1$ проходов по всем $E$ рёбрам.

На графе лекции этот механизм виден в числах: кратчайший путь до вершины 4 — это $0 \xrightarrow{7} 2 \xrightarrow{-3} 3 \xrightarrow{-2} 1 \xrightarrow{-4} 4$, ровно $4 = V - 1$ рёбер. Инвариант гарантирует: $\text{dist}[2] = 7$ финально после итерации 1, $\text{dist}[3] = 4$ — после 2, $\text{dist}[1] = 2$ — после 3, $\text{dist}[4] = -2$ — после 4. В трассировке выше алгоритм сошёлся уже за две итерации — но лишь потому, что порядок рёбер оказался удачным и релаксации каскадировали; оценка $V-1$ — гарантия для *любого* порядка.

![Форд-Беллман: индукция по длине кратчайшего пути](assets/bellman_ford_induction.png)

На рисунке — тот же граф, но рёбра нарочно переставлены в «плохой» порядок, при котором каскад внутри итерации не помогает. Сверху — цепочка кратчайшего пути $0 \to 2 \to 3 \to 1 \to 4$: под каждой вершиной подписана итерация, на которой её расстояние стало финальным, — ровно одно ребро пути за итерацию, в точности как в индукционном переходе. Снизу — таблица `dist` по итерациям (оранжевым — изменившиеся ячейки): теперь алгоритму нужны все $V - 1 = 4$ итерации. В этом и смысл оценки: за итерацию фронт гарантированно продвигается на одно ребро вдоль каждого кратчайшего пути, а быстрее — как повезёт с порядком рёбер.

**Почему $V$-я итерация ловит отрицательный цикл.** Если отрицательных циклов нет, то после $V-1$ итераций $\text{dist} = \delta$, и ни одна релаксация больше не сработает: для любого ребра $(u,v)$ выполнено $\delta(s,v) \le \delta(s,u) + w(u,v)$ (неравенство треугольника — путь через $u$ не короче кратчайшего). Обратно, пусть есть достижимый отрицательный цикл $v_1 \to v_2 \to \cdots \to v_k \to v_1$, и предположим, что на $V$-й итерации ни одно его ребро не расслабилось: $\text{dist}[v_{i+1}] \le \text{dist}[v_i] + w(v_i, v_{i+1})$ для всех $i$. Сложим все $k$ неравенств по циклу: суммы $\sum \text{dist}[v_i]$ слева и справа сократятся, останется $0 \le \sum w < 0$ — противоречие. Значит, хотя бы одна релаксация на цикле обязана сработать, и алгоритм честно сообщает о цикле.
...
На практике измерение $k$ убирают, работая на месте, в одной матрице. Почему это не ломает динамику? На фазе $k$ опасность одна: вдруг при вычислении $d[i][j]$ мы прочитаем $d[i][k-1]$ или $d[k-1][j]$, уже обновлённые *текущей* фазой, а не предыдущей. Но эти значения на фазе $k$ вообще не меняются: обновление для них имело бы вид $d[i][k-1] \gets \min(d[i][k-1],\ d[i][k-1] + d[k-1][k-1])$, а диагональный элемент $d[k-1][k-1]$ равен нулю (пока нет отрицательных циклов) — прибавка нуля ничего не улучшает. Строка и столбец с номером $k-1$ на своей фазе «заморожены», поэтому старая и новая версии совпадают, и трёхмерный массив можно безболезненно схлопнуть в двумерный.
...
После $k = 0$ (пути через вершину 0): $d[3][1] = d[3][0] + d[0][1] = 2 + 3 = 5$; $d[2][1] = 5 + 3 = 8$; $d[1][3] = 8 + 7 = 15$.

После $k = 1$ (плюс вершина 1): $d[0][2] = d[0][1] + d[1][2] = 3 + 2 = 5$; $d[3][2] = d[3][1] + d[1][2] = 5 + 2 = 7$.

После $k = 2$ (плюс вершина 2): $d[0][3] = \min(7,\ d[0][2] + d[2][3]) = \min(7,\ 5 + 1) = 6$; $d[1][3] = \min(15,\ 2 + 1) = 3$; $d[1][0] = \min(8,\ d[1][2] + d[2][0]) = \min(8,\ 2 + 5) = 7$.

После $k = 3$ (плюс вершина 3): $d[1][0] = \min(7,\ d[1][3] + d[3][0]) = \min(7,\ 3 + 2) = 5$ — путь $1 \to 2 \to 3 \to 0$; $d[2][0] = \min(5,\ 1 + 2) = 3$; $d[2][1] = \min(8,\ d[2][3] + d[3][1]) = \min(8,\ 1 + 5) = 6$. Диагональ осталась нулевой (например, $d[0][0] = \min(0,\ 6 + 2) = 0$) — отрицательных циклов нет.
...
Обратите внимание на клетку $d[1][0]$: прямое ребро $1 \to 0$ весит 8, но итоговое расстояние — 5 (путь $1 \to 2 \to 3 \to 0$ длины $2+1+2$). Улучшение пришло в два шага: сначала через промежуточную вершину 2, затем через 3 — именно так и накапливается ответ в динамике по $k$.
...
![Алгоритм Дийкстры: три первых шага на графе из 6 вершин](assets/dijkstra_progression.png)

На рисунке — первые три шага этой же трассировки. Зелёные вершины закреплены (их расстояние финально), оранжевые — «граница» (frontier): для них уже есть конечная оценка $d$, но она ещё может улучшиться, серые — недостижимые пока вершины с $d = \infty$. Проследите за вершиной 3: на шаге 2 она получает оценку $d = 9$ (через ребро $1 \to 3$ веса 7), но закрепится позже со значением 8 — через путь $0 \to 1 \to 2 \to 4 \to 3$. Это иллюстрирует главное правило: оценка вершины на границе — ещё не ответ; ответом она становится только в момент извлечения вершины с минимальным $d$.

### Контрпример: отрицательное ребро

Возьмём граф из трёх вершин: $0 \to 1$ (вес 2), $0 \to 2$ (вес 3), $2 \to 1$ (вес $-2$). Истинное расстояние до вершины 1 равно $3 + (-2) = 1$ (путь через 2). Дийкстра же действует так: закрепляет 0, затем закрепляет 1 с $d = 2$ (минимальная оценка), затем закрепляет 2 с $d = 3$ и обнаруживает ребро $2 \to 1$, дающее $3 - 2 = 1 < 2$ — но вершина 1 уже закреплена, и в классической формулировке её расстояние не пересматривается. Ответ $d[1] = 2$ неверен. Жадный аргумент корректности («дальше путь может только удлиняться») опирается на неотрицательность весов и здесь ломается.

![Контрпример: Дийкстра на графе с отрицательным ребром](assets/dijkstra_negative.png)

На рисунке этот контрпример: слева порядок закрепления вершин и ошибочный ответ $d[1]=2$, справа — истинный кратчайший путь $0 \to 2 \to 1$ длины 1, который алгоритм упускает, потому что закрепил вершину 1 до того, как обработал отрицательное ребро.

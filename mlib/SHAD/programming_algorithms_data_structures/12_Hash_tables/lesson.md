...
std::vector<int> rabin_karp(const std::string& text, const std::string& pattern) {
    ...
    // Степень p^m для удаления старого символа
    long long p_m = 1;
    for (int i = 0; i < m; ++i) p_m = p_m * P % MOD;
    ...
    for (int i = 0; i < m; ++i) {
        ...
        window_hash  = (window_hash  + (text[i]    - 'a' + 1) * p_pow) % MOD;
        p_pow = p_pow * P % MOD;
    }
    ...
    for (int i = m; i < n; ++i) {
        // Убираем левый символ, добавляем правый
        window_hash = (window_hash - (text[i - m] - 'a' + 1) + MOD) % MOD;
        window_hash = window_hash * 700000006LL % MOD;  // умножение на P^{-1} mod MOD
        window_hash = (window_hash + (text[i] - 'a' + 1) * p_pow) % MOD;

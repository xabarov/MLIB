...
```bash
docker compose -f apps/interactive/docker-compose.dev.yml config --quiet
docker compose -f apps/interactive/docker-compose.yml config --quiet
```
...
```bash
SHAD_INTERACTIVE_URL=http://127.0.0.1:5173 .venv/bin/python apps/interactive/scripts/smoke_playwright.py
```
...
```bash
make interactive-build
SHAD_INTERACTIVE_URL=<preview-url> .venv/bin/python apps/interactive/scripts/smoke_playwright.py
```

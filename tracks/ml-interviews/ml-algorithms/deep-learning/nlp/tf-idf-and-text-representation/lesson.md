...
Именно это и породило [плотные векторные представления](../word-embeddings/lesson.md): если сопоставить словам обучаемые векторы небольшой размерности, «машина» и «автомобиль» смогут оказаться рядом. TF-IDF при этом никуда не делся — он остаётся сильной базой, дешёвым и интерпретируемым решением, а гибрид «BM25 + плотный поиск» до сих пор работает лучше каждой части по отдельности.
...
- [Векторные представления слов](../word-embeddings/lesson.md) — чем лечится потолок мешка слов

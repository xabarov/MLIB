...
**Ключевое наблюдение.** Очередь FIFO выдаёт вершины в том порядке, в котором они были добавлены, а каждая новая метка равна метке извлечённой вершины плюс один. Отсюда **инвариант очереди**: в любой момент метки `dist` вершин в очереди идут в неубывающем порядке и различаются не более чем на 1 — очередь всегда имеет вид $[d, \dots, d, d{+}1, \dots, d{+}1]$.

**Индукция.** База: очередь $[s]$, единственная метка 0 — инвариант тривиален. Переход: пусть инвариант верен; извлекаем вершину $u$ с минимальной меткой $d$ (она стоит первой). После извлечения в очереди остались метки из $\{d, d{+}1\}$, а все добавляемые соседи получают метку $d+1$ — не меньше любой оставшейся и не дальше чем на 1 от минимума. Инвариант сохранился.

**Следствие (монотонность).** Вершины извлекаются в порядке неубывания `dist`: слой $d$ обрабатывается полностью раньше слоя $d+1$. На трассировке выше метки в очереди меняются так: $(0) \to (1,1) \to (1,2,2) \to (2,2,2,2)$ — ни разу не убывают и никогда не расходятся больше чем на 1.

**Почему $\text{dist}[v] = d(s,v)$ — именно кратчайшее расстояние.** Два неравенства:

- $\text{dist}[v] \ge d(s,v)$: метка — длина реального пути $s \to \dots \to \text{parent}[v] \to v$, а кратчайший путь не длиннее любого реального.
- $\text{dist}[v] \le d(s,v)$ — индукция по истинному расстоянию. Пусть все вершины с $d(s,\cdot) = d$ получили метку ровно $d$. Возьмём $v$ с $d(s,v) = d+1$ и её соседа $u$ на кратчайшем пути: $d(s,u) = d$, значит $\text{dist}[u] = d$. В момент извлечения $u$ вершина $v$ либо ещё не помечена — тогда получает метку $d+1$, либо уже помечена — но метки выдаются в неубывающем порядке, поэтому её метка $\le d+1$.

Вместе: $\text{dist}[v] = d(s,v)$ для всех достижимых вершин.

![Инвариант очереди BFS: трассировка всех шагов](assets/bfs_queue_invariant.png)

На диаграмме — полная трассировка BFS от вершины 0 на графе из примера: слева извлекаемая вершина, справа содержимое очереди после шага (зелёным — только что добавленные вершины, под каждой — её метка $d$). Видно оба свойства инварианта: метки внутри очереди никогда не убывают, и в очереди одновременно живут не более двух соседних значений $d$ и $d+1$ — граница между «текущим» и «следующим» слоем движется слева направо.

**Завершаемость и сложность из структуры.** Каждая вершина помечается и попадает в очередь не более одного раза (проверка `dist[v] == -1` **при добавлении**), поэтому цикл `while` выполняется не более $V$ раз, а каждое ребро просматривается по одному разу с каждого конца — итого $O(V+E)$. Отсюда же видно, почему нельзя помечать вершину при извлечении (типичная ошибка 1 из раздела 7): без ранней пометки одна вершина попадает в очередь несколько раз, и инвариант очереди ломается.
...
**Почему обратное ребро — точный критерий цикла.** В одну сторону: серые вершины — это в точности текущий путь рекурсии (DFS вошёл, но ещё не вышел), то есть цепочка предков по древесным рёбрам. Если из $u$ ведёт ребро в серую вершину $v$, то древесный путь $v \to \dots \to u$ вместе с ребром $u \to v$ замыкает ориентированный цикл — в примере из раздела 4 это серая цепочка $0 \to 1 \to 3$ и ребро $3 \to 0$, дающие цикл $0 \to 1 \to 3 \to 0$. В другую сторону: пусть цикл есть, и $c$ — та его вершина, которую DFS открыл первой. В момент открытия $c$ остальные вершины цикла белые и достижимы из $c$ по рёбрам цикла, поэтому все они будут открыты и закрыты **до** выхода из $c$ (лемма о белом пути). В частности, ребро цикла, входящее в $c$, будет просмотрено, пока $c$ ещё серая, — и проверка `color[v] == 1` сработает. Значит, алгоритм не может ни выдать ложный цикл, ни пропустить настоящий.

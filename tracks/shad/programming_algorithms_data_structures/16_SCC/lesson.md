...
```
Рёбра: 0→1, 1→2, 2→0,   (цикл — SCC A = {0,1,2})
       4→5, 5→4,          (цикл — SCC B = {4,5})
       6→7, 7→6,          (цикл — SCC C = {6,7})
       2→3, 4→3, 3→6, 5→6   (рёбра между компонентами)
```

Вершина 3 достижима из SCC A (через $2 \to 3$), но из вершины 3 нет пути обратно в 0, 1 или 2: единственное исходящее ребро $3 \to 6$ уводит «вперёд», в компоненту C. Взаимной достижимости нет — значит, вершина 3 — отдельная тривиальная SCC D = {3}.

Итого: **4 компоненты** — A={0,1,2}, B={4,5}, C={6,7}, D={3}.

![Сильно связные компоненты орграфа](assets/sccs.png)

На рисунке каждая SCC выделена цветной областью. Сплошные цветные стрелки — рёбра **внутри** компонент: именно они образуют циклы, обеспечивающие взаимную достижимость (треугольник $0 \to 1 \to 2 \to 0$, двойки $4 \leftrightarrow 5$ и $6 \leftrightarrow 7$). Пунктирные чёрные стрелки — рёбра **между** компонентами: по ним можно уйти из одной SCC в другую, но вернуться обратно нельзя. Убедитесь, что среди пунктирных рёбер нет ни одного «встречного» — иначе две компоненты слились бы в одну.
...
Для нашего примера конденсация — DAG из 4 узлов с рёбрами:

$$
A \to D \quad (2 \to 3), \qquad B \to D \quad (4 \to 3), \qquad D \to C \quad (3 \to 6), \qquad B \to C \quad (5 \to 6).
$$

Источники конденсации (in-degree = 0) — компоненты $A$ и $B$, единственный сток (out-degree = 0) — компонента $C$.

![Конденсация орграфа: исходный граф и DAG](assets/condensation.png)

Слева — исходный орграф с раскраской по компонентам, справа — его конденсация. Каждая цветная группа вершин «сжалась» в один узел-прямоугольник, а из нескольких параллельных рёбер между парами компонент осталось по одному. Обратите внимание: в конденсации нет ни одного цикла — все стрелки ведут «слева направо» от источников $A$ и $B$ к стоку $C$, как и обещает теорема.
...
Рёбра $G$: 0→1, 1→2, 2→0, 2→3, 4→3, 4→5, 5→4, 6→7, 7→6, 3→6, 5→6.

Проход 1 (DFS по $G$; запускаем из непосещённых вершин по возрастанию номера):
- DFS(0): идём $0 \to 1 \to 2$; ребро $2 \to 0$ ведёт в посещённую вершину; дальше $2 \to 3 \to 6 \to 7$; ребро $7 \to 6$ — в посещённую. Начинается «раскрутка»: завершаются 7, затем 6, затем 3, затем 2, 1, 0.
- DFS(4): вершина 3 уже посещена; идём $4 \to 5$; из 5 всё посещено. Завершаются 5, затем 4.
- Порядок завершения (вектор `order` слева направо): **7, 6, 3, 2, 1, 0, 5, 4**.

Проход 2 (DFS по $G^T$; берём вершины в порядке **убывания** времени завершения, т.е. вектор `order` с конца):
- Берём вершину 4 (завершилась последней): DFS(4) в $G^T$ находит {4, 5} → SCC B. Уйти дальше нельзя: ребро $3 \to 4$ в $G^T$ ведёт из 3 в 4, а не наоборот.
- Берём вершину 5 — уже посещена. Берём вершину 0: DFS(0) в $G^T$ находит {0, 1, 2} → SCC A.
- Берём вершину 3: DFS(3) в $G^T$ — соседи 2 и 4 уже посещены → SCC D = {3}.
- Берём вершину 6: DFS(6) в $G^T$ находит {6, 7} → SCC C.

![Алгоритм Косарайю: два прохода DFS](assets/kosaraju.png)

Слева — исходный граф $G$; число возле вершины показывает, какой по счёту она **завершилась** в первом проходе (7 завершилась первой, 4 — последней, восьмой). Справа — транспонированный граф $G^T$: все стрелки развёрнуты, а цвет вершины — SCC, найденная во втором проходе. Второй проход стартует с вершины 4 (максимальное время завершения) и «подхватывает» ровно свою компоненту {4, 5}: в $G^T$ рёбра, которые в $G$ выходили из компоненты B, теперь входят в неё, и убежать в чужую SCC невозможно.
...
### Почему алгоритм Косарайю корректен

**Ключевое наблюдение (лемма о максимальном $t_{out}$).** Для компоненты $C$ обозначим $f(C) = \max_{v \in C} t_{out}[v]$ — время завершения самой «поздней» её вершины в первом проходе. Тогда если в конденсации есть ребро $C \to C'$, то $f(C) > f(C')$.

*Почему.* Посмотрим, в какую из двух компонент DFS первого прохода зашёл раньше.
- Раньше открыта вершина $x \in C$. В этот момент все вершины $C$ и $C'$ белые и достижимы из $x$: внутри $C$ — по сильной связности, в $C'$ — через ребро между компонентами. По лемме о белом пути все они закроются до выхода из $x$, значит $t_{out}[x] = f(C) > f(C')$.
- Раньше открыта вершина из $C'$. Из $C'$ нет пути в $C$ (иначе в конденсации был бы цикл, а она — DAG), поэтому DFS закроет всю $C'$, не заходя в $C$; каждая вершина $C$ откроется и закроется позже — снова $f(C) > f(C')$.

Иными словами, **сортировка компонент по убыванию $f$ — это готовый топологический порядок конденсации.** Проверим на нашем примере (номера завершения из трассировки: 7, 6, 3, 2, 1, 0, 5, 4): $f(B) = 8$ (вершина 4 завершилась восьмой), $f(A) = 6$ (вершина 0), $f(D) = 3$, $f(C) = 2$. Все четыре ребра конденсации — $A \to D$, $B \to D$, $D \to C$, $B \to C$ — идут от большего $f$ к меньшему: $6 > 3$, $8 > 3$, $3 > 2$, $8 > 2$.

![Лемма Косарайю: рёбра конденсации идут от большего f к меньшему](assets/kosaraju_lemma.png)

На диаграмме — конденсация нашего графа, где у каждой компоненты подписано её значение $f$ (максимальный номер завершения, вычисленный первым проходом DFS). Каждая стрелка ведёт строго от большей метки к меньшей — это и есть лемма в действии: убывание $f$ вдоль всех рёбер означает, что порядок «по убыванию $f$» ($B, A, D, C$) — топологический порядок конденсации.

**Почему второй проход собирает ровно одну SCC.** Индукция по запускам DFS в $G^T$. Первая стартовая вершина $v$ имеет максимальный $t_{out}$ во всём графе, значит $f(SCC(v))$ максимален, и по лемме в $SCC(v)$ не входит ни одно ребро конденсации (у источника такого ребра $f$ был бы ещё больше). Множество вершин, достижимых из $v$ в $G^T$, — это в точности вершины, из которых $v$ достижима в $G$: сама $SCC(v)$ плюс компоненты с путём **в** $SCC(v)$, а таких нет. Поэтому DFS помечает ровно $SCC(v)$: не больше (убежать некуда) и не меньше (внутри SCC все вершины взаимодостижимы и в $G$, и в $G^T$). Шаг индукции: перед очередным запуском все ранее найденные компоненты уже помечены и для DFS «невидимы»; стартовая вершина имеет максимальный $t_{out}$ среди оставшихся, значит её компонента — источник конденсации оставшейся части графа, и тот же аргумент проходит дословно. В примере: старт с вершины 4 ($t_{out}$ максимален) даёт {4, 5}; следующая непосещённая по порядку — вершина 0, и DFS по $G^T$ собирает {0, 1, 2}, не задевая помеченные 4 и 5; затем {3} и {6, 7}.

**Завершаемость и сложность.** Оба прохода — обычные DFS по всем вершинам: каждая вершина и каждое ребро обрабатываются по одному разу в $G$ и по одному разу в $G^T$; построение $G^T$ — ещё один линейный проход по списку рёбер. Итого $O(V + E)$.
...
**Трассировка на том же графе** (рёбра: 0→1, 1→2, 2→0, 2→3, 4→3, 4→5, 5→4, 6→7, 7→6, 3→6, 5→6):

Стартуем из вершины 0:
- `dfs(0)`: disc[0]=0, low[0]=0, стек=[0]
  - `dfs(1)`: disc[1]=1, low[1]=1, стек=[0,1]
    - `dfs(2)`: disc[2]=2, low[2]=2, стек=[0,1,2]
      - ребро 2→0: 0 на стеке → low[2]=min(2, disc[0])=0
      - ребро 2→3: переходим по дереву
        - `dfs(3)`: disc[3]=3, low[3]=3, стек=[0,1,2,3]
          - ребро 3→6: переходим по дереву
            - `dfs(6)`: disc[6]=4, low[6]=4, стек=[0,1,2,3,6]
              - `dfs(7)`: disc[7]=5, low[7]=5, стек=[0,1,2,3,6,7]
                - ребро 7→6: 6 на стеке → low[7]=min(5, disc[6])=4
                - low[7]=4 ≠ disc[7]=5 → не корень
              - low[6]=min(4, low[7])=4 == disc[6]=4 → **корень SCC!** Выталкиваем 7, 6 → **SCC C = {6,7}**
          - low[3]=min(3, low[6])=3 == disc[3]=3 → **корень SCC!** Выталкиваем 3 → **SCC D = {3}**
        - low[2]=min(0, low[3])=0 ≠ disc[2]=2 → не корень
    - low[1]=min(1, low[2])=0 ≠ disc[1]=1 → не корень
  - low[0]=min(0, low[1])=0 == disc[0]=0 → **корень SCC!** Выталкиваем 2, 1, 0 → **SCC A = {0,1,2}**

Затем `dfs(4)`: disc[4]=6, low[4]=6, стек=[4]
- ребро 4→3: вершина 3 посещена, но **не на стеке** (уже вытолкнута как SCC D) → игнорируем, low не трогаем
- `dfs(5)`: disc[5]=7, low[5]=7, стек=[4,5]
  - ребро 5→4: 4 на стеке → low[5]=min(7, disc[4])=6
  - ребро 5→6: вершина 6 не на стеке → игнорируем
  - low[5]=6 ≠ disc[5]=7 → не корень
- low[4]=min(6, low[5])=6 == disc[4]=6 → **корень SCC!** Выталкиваем 5, 4 → **SCC B = {4,5}**

Обратите внимание на два поучительных момента. Во-первых, компоненты выталкиваются в порядке «стоки первыми»: C, D, A, B — компонента C нашлась раньше всех, хотя обход начался в A. Во-вторых, шаг «ребро 4→3: вершина посещена, но не на стеке — игнорируем» демонстрирует, зачем нужен флаг `onStack`: без него low[4] ошибочно уменьшился бы через чужую, уже закрытую компоненту.
...
| Порядок SCC | «Источники» первыми (нумерация — топологический порядок конденсации) | «Стоки» первыми (нумерация — обратный топологический порядок) |
...
**Топологический порядок конденсации для нашего примера.** Тарьян нумерует компоненты в порядке выталкивания («стоки первыми»): C={6,7}→0, D={3}→1, A={0,1,2}→2, B={4,5}→3.
- Рёбра конденсации в этих номерах: $2 \to 1$ (A→D, из ребра 2→3), $3 \to 1$ (B→D, из 4→3), $1 \to 0$ (D→C, из 3→6), $3 \to 0$ (B→C, из 5→6).
- Алгоритм Кана: источники — компоненты 2 (A) и 3 (B); порядок, например, $[2, 3, 1, 0]$, то есть **A, B, D, C**. Порядок $B, A, D, C$ тоже корректен — между A и B рёбер нет.
- Полезный факт: обратная нумерация Тарьяна $(3, 2, 1, 0) = B, A, D, C$ — уже готовый топологический порядок конденсации, отдельная сортировка не обязательна.

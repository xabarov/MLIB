...
1. От линейных операторов к нормальным формам
2. Минимальное повторение: собственные векторы и диагонализация
3. Характеристический многочлен
4. Теорема Гамильтона–Кэли
5. Что теорема Гамильтона–Кэли даёт на практике
...
14. Практический алгоритм и восстановление размеров жордановых клеток
...
В прошлой лекции линейный оператор уже рассматривался как один и тот же геометрический объект, записанный в разных базисах. Если в старом базисе его матрица равна $A$, а в новом — $B$, то
$$
B=C^{-1}AC.
$$

Значит, главный вопрос теперь такой: **можно ли подобрать базис, в котором матрица оператора выглядит максимально просто?**
...
Но не всякий оператор диагонализуем. Тогда следующим естественным нормальным видом становится **жорданова форма**. Она не делает матрицу полностью диагональной, но оставляет от недиагонализуемости только очень простые блоки.
...
Перед жордановой формой нужно ввести главный язык линейных операторов: собственные векторы. Подробная тема собственных значений будет ещё выделена отдельно дальше в курсе, но для теоремы Гамильтона–Кэли и жордановой формы нужен именно этот минимум.

Это сделано намеренно: порядок лекций следует программе для поступающих, где Гамильтон–Кэли и жорданова форма идут раньше отдельного блока про собственные значения. Поэтому здесь мы берём только то, без чего нельзя говорить о нормальной форме оператора; позже, в лекции о собственных векторах, эти понятия будут собраны в полный самостоятельный аппарат.
...
### 5.3. Почему здесь начинается путь к жордановой форме

Теорема Гамильтона–Кэли говорит, что у оператора обязательно есть ненулевой многочлен, который его зануляет. Это не просто вычислительный трюк: оператор можно изучать через многочлены от него.

Самый простой случай такого поведения — нильпотентный оператор. Для него зануляющий многочлен имеет вид $t^m$, то есть после нескольких применений оператор становится нулевым. Именно нильпотентные операторы дают чистую модель того, как устроены жордановы цепочки.
...
Это не определение общего вида нильпотентной матрицы, а удобный первый пример. В исходном базисе нильпотентный оператор может выглядеть иначе: например,
$$
\begin{pmatrix}
0 & 2\\
0 & 0
\end{pmatrix}
$$
тоже нильпотентна. Жорданова теория как раз утверждает, что после подходящей замены базиса нильпотентный оператор распадается на такие простые блоки с нулями на диагонали.
...
Здесь порядок базиса важен. Мы записываем цепочку как
$$
(\varphi-\lambda E)v_1=0,\qquad
(\varphi-\lambda E)v_2=v_1,\qquad \dots,\qquad
(\varphi-\lambda E)v_k=v_{k-1}.
$$
Тогда
$$
\varphi(v_j)=\lambda v_j+v_{j-1}\quad (j\ge 2),
$$
и в столбце матрицы для $v_j$ появляется единица в строке $v_{j-1}$. Поэтому единицы стоят **над** главной диагональю. На рисунке цепочка читается от верхнего вектора к собственному:
$$
v_k\to v_{k-1}\to\dots\to v_1\to 0,
$$
а в матрицу базис удобно ставить в порядке $v_1,\dots,v_k$.
...
Разберём эту запись медленно.

Каждое множество $\ker N^k$ состоит из тех векторов, которые оператор $N$ убивает не позже чем за $k$ шагов:
$$
\ker N^k=\{v\in V\mid N^k v=0\}.
$$

Например:

- $\ker N$ — векторы, которые сразу переходят в нуль;
- $\ker N^2$ — векторы, которые становятся нулём после не более чем двух применений $N$;
- $\ker N^3$ — векторы, которым нужно не более трёх шагов;
- и так далее.

Почему это именно **возрастающая цепочка** подпространств?  
Потому что каждый следующий член содержит предыдущий. Если $v\in \ker N^k$, то
$$
N^k v=0.
$$
Тогда автоматически
$$
N^{k+1}v = N(N^k v)=N(0)=0,
$$
значит,
$$
v\in \ker N^{k+1}.
...
Следовательно,
$$
\ker N^k\subseteq \ker N^{k+1}.
$$

Слово "цепочка" здесь означает просто последовательность вложенных подпространств:
$$
U_1\subseteq U_2\subseteq U_3\subseteq \dots
$$
Слово "возрастающая" не означает, что размерность обязана увеличиваться на каждом шаге. Иногда два соседних подпространства могут совпасть. Возрастание означает только то, что мы не теряем векторы: каждый старый вектор остаётся и на следующем уровне.

Полезно представлять себе эту цепочку как разбиение пространства по "высоте" векторов:

- в $\ker N$ лежат векторы высоты $1$;
- в $\ker N^2\setminus \ker N$ лежат векторы высоты ровно $2$;
- в $\ker N^3\setminus \ker N^2$ лежат векторы высоты ровно $3$;
- вообще в $\ker N^k\setminus \ker N^{k-1}$ лежат векторы, которые зануляются ровно за $k$ шагов.

Именно поэтому после шага 1 становится понятнее шаг 2: разность
$$
\dim \ker N^k-\dim \ker N^{k-1}
$$
считает, сколько **новых независимых направлений** появилось на уровне $k$, то есть сколько есть векторов высоты не меньше $k$.
...
Что такое **верхушка** цепочки?

Если есть жорданова цепочка длины $k$
$$
v_k \xrightarrow{N} v_{k-1} \xrightarrow{N} \dots \xrightarrow{N} v_2 \xrightarrow{N} v_1 \xrightarrow{N} 0,
$$
то "верхушкой" называют самый верхний вектор $v_k$.

Почему именно его?

- он находится дальше всех от нуля;
- для него
$$
N^k v_k=0,\qquad N^{k-1}v_k\ne 0;
$$
- если начать с него и многократно применять $N$, то восстанавливается вся цепочка:
$$
v_k,\ Nv_k,\ N^2v_k,\dots, N^{k-1}v_k.
$$

То есть верхушка — это вектор максимальной высоты в данной цепочке. Все остальные элементы цепочки получаются из неё последовательными применениями оператора.

В терминах шага 1 верхушки длины $k$ нужно искать среди векторов из
$$
\ker N^k\setminus \ker N^{k-1}.
$$
Действительно, такие векторы убиваются за $k$ шагов, но не убиваются за $k-1$ шагов, значит они как раз могут служить началом цепочки длины $k$.

А если говорить совсем формально, то разные независимые верхушки на уровне $k$ соответствуют независимым элементам факторпространства
$$
\ker N^k/\ker N^{k-1}.
$$
Именно поэтому шаг 2 считает количество цепочек длины хотя бы $k$.
...
Практически это удобно оформлять таблицей.

| Что считаем | Формула | Смысл |
|---|---|---|
| $d_k$ | $\dim\ker N^k$ | сколько векторов зануляется не позже чем за $k$ шагов |
| $b_{\ge k}$ | $d_k-d_{k-1}$ | сколько жордановых блоков имеют длину хотя бы $k$ |
| $b_{=k}$ | $b_{\ge k}-b_{\ge k+1}$ | сколько блоков имеют длину ровно $k$ |

То есть сначала считаются ядра, затем их первые разности, а потом разности этих разностей. На экзамене это часто короче, чем пытаться строить весь жорданов базис.
...
Если дана матрица $A$, общий путь такой:
...
2. Найти собственные значения как корни уравнения
   ...
   \chi_A(t)=0.
   $$
   Для каждого корня сразу отметить его **алгебраическую кратность**.

3. Для каждого собственного значения $\lambda$ решить систему
   $$
   (A-\lambda E)x=0.
   $$
   Её пространство решений равно собственному подпространству
   $$
   E_\lambda=\ker(A-\lambda E).
   $$

4. Проверить, хватает ли собственных векторов. Если сумма размерностей всех $E_\lambda$ равна $n$, то матрица диагонализуема.

5. Если собственных векторов не хватает, считать ядра степеней:
   $$
   \ker(A-\lambda E),\quad \ker(A-\lambda E)^2,\quad \ker(A-\lambda E)^3,\dots
   $$
   Они показывают размеры жордановых блоков для данного $\lambda$.

Коротко: характеристический многочлен даёт собственные значения, обычные ядра дают собственные векторы, а ядра степеней дают недостающую жорданову структуру.

...
1. Зачем расширять действительные числа
2. Алгебраическая форма $a+bi$ и операции
3. Комплексная плоскость, модуль и аргумент
4. Тригонометрическая форма и формула Муавра
5. Извлечение корней
6. Корни из единицы и первообразные корни

После первой лекции мы уже видели, что алгебра часто изучает не отдельные объекты, а операции над ними. Подстановки были конечными биекциями с операцией композиции. Комплексные числа — другой пример расширения языка: мы добавляем число $i$ и получаем систему, где можно складывать, умножать, делить и решать больше уравнений, чем в $\mathbb R$.
...
Важно: по одному только значению тангенса аргумент выбирать нельзя. Например, у чисел $1+i$ и $-1-i$ одинаковое отношение $b/a=1$, но они лежат в разных четвертях и имеют разные направления. Поэтому при переходе к аргументу всегда нужно смотреть на знаки $a$ и $b$.
...
Почему достаточно именно этих $n$ значений $k$? Если увеличить $k$ на $n$, то угол увеличится на $2\pi$, а точка на комплексной плоскости не изменится. Поэтому дальше начнут повторяться уже найденные корни.
...
Это очень близко к понятию порядка из первой лекции: там порядок подстановки был минимальным числом применений, после которого подстановка возвращается к тождественной. Здесь порядок корня из единицы — минимальное $m$, для которого $\zeta^m=1$. Первообразный корень $n$-й степени — это корень порядка ровно $n$.
...
Почему появляется условие $\gcd(k,n)=1$? Степени корня $\zeta_k$ дают углы
$$
\frac{2\pi k}{n},\quad \frac{2\pi\cdot 2k}{n},\quad \frac{2\pi\cdot 3k}{n},\dots
$$
То есть по окружности мы каждый раз делаем шаг на $k$ делений из $n$. Если $k$ и $n$ имеют общий делитель, такой шаг попадает только в часть вершин. Если же $\gcd(k,n)=1$, шаг рано или поздно проходит все вершины правильного $n$-угольника.

![Точная схема: первообразные корни шестой степени из единицы — это корни с индексами, взаимно простыми с 6](assets/primitive_roots_six.png)

*Рис. 6. Для $n=6$ первообразны только корни с $k=1$ и $k=5$.*
...
Дальше эти числа и операции будут использоваться уже как обычная алгебраическая среда. В следующей лекции мы перейдём к системам линейных уравнений: там важны не столько сами комплексные числа, сколько привычка записывать объекты координатами и выполнять с ними операции по строгим правилам.

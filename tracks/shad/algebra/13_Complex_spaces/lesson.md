...
Предыдущая лекция показала, что собственные значения и диагонализация зависят от поля: над $\mathbb{R}$ у матрицы может не хватать корней характеристического многочлена. Поэтому теперь мы переходим к комплексным пространствам и одновременно аккуратно переносим туда евклидову геометрию.

Главная линия лекции:
$$
\mathbb{C}\text{-пространства}\to \text{эрмитовы формы}\to \text{эрмитова геометрия}\to \text{нормальные и самосопряжённые операторы}\to \text{SVD}.
$$

Как читать эту лекцию:

- разделы 2--5 дают обязательный язык комплексных пространств и эрмитовых форм;
- разделы 6--7 — вычислительный блок про Якоби, Сильвестра и знаки форм;
- разделы 8--10 — геометрия и спектральная теорема;
- раздел 11 про SVD нужен как важное расширение к анализу данных: понимать формулировку и связь с $A^*A$ важнее, чем вручную строить полное SVD для больших матриц.
...
Ключевая новая техническая деталь — комплексное сопряжение в одном из аргументов. Без него выражение, похожее на квадрат длины, перестаёт быть положительным.
...
Короткое соответствие с предыдущими лекциями:

| Вещественный случай | Комплексный случай |
|---|---|
| симметрическая билинейная форма | эрмитова полуторалинейная форма |
| $x^TAy$ при соглашении без сопряжения | $y^*Ax$ при нашем соглашении |
| скалярное произведение: положительно определённая симметрическая форма | эрмитово скалярное произведение: положительно определённая эрмитова форма |
| ортогональная матрица $Q^TQ=I$ | унитарная матрица $U^*U=I$ |
...
H(x,y)=y^*Ax,
$$
где $y^*$ — сопряжённо-транспонированный вектор-строка, а матрица $A$ удовлетворяет
$$
A^*=A.
$$
...
Эта запись согласована с нашим соглашением: форма линейна по первому аргументу и сопряжённо-линейна по второму. В курсах, где скалярное произведение линейно по второму аргументу, ту же идею часто записывают как $x^*Ay$.
...
Этот раздел временно возвращает нас к вещественным квадратичным формам из предыдущей лекции. Он нужен здесь как вычислительный мост: те же идеи положительной определённости и знаков потом применяются к эрмитовым матрицам, только с сопряжённым транспонированием вместо обычного.
...
Нормальность — это условие совместимости оператора со своим сопряжённым. Оно важно не само по себе, а потому что именно нормальные операторы в комплексном эрмитовом пространстве имеют ортонормированный базис из собственных векторов.
...
Во втором равенстве мы уже используем, что собственные значения самосопряжённого оператора вещественны. При нашем соглашении скалярное произведение сопряжённо-линейно по второму аргументу, поэтому в общем случае появился бы множитель $\overline{\mu}$.
...
Важно отделять SVD от диагонализации. Диагонализация требует квадратного оператора и собственных векторов самой матрицы $A$. SVD существует для любой прямоугольной матрицы и строится через самосопряжённые матрицы $A^*A$ и $AA^*$.
...
Практическая проверка памяти: собственные значения могут быть отрицательными или комплексными, а сингулярные числа всегда неотрицательны. Поэтому фраза “сингулярные числа — это собственные значения” неверна; правильно: $\sigma_k^2$ — собственные значения матрицы $A^*A$.

---

### Расширение: применения в машинном обучении и анализе данных

Этот блок не должен сбивать основной фокус лекции. Для поступления достаточно понимать, что SVD существует для любой матрицы, сингулярные числа связаны с $A^*A$, а усечённое SVD даёт лучшее низкоранговое приближение. Примеры ниже показывают, почему эта алгебра постоянно появляется в анализе данных.
...
### SVD и прикладной минимум

- понимать формулировку SVD и компактного SVD;
- знать связь сингулярных чисел с собственными значениями $A^*A$;
- уметь находить сингулярные числа в простых примерах;
- знать смысл усечённого SVD и теоремы Экарта--Янга;
- понимать, что PCA и низкоранговое приближение данных выражаются через SVD.

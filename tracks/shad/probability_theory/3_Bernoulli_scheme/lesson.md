...
### Пример: когда Пуассон уместен, а когда нет

**Уместен:** завод выпускает $n = 10000$ деталей, вероятность брака $p = 0.0003$. $\lambda = np = 3 \ll n$, $p$ мало → $X \approx \mathrm{Poisson}(3)$.

$$\mathbb{P}(X = 0) \approx e^{-3} \approx 0.050; \quad \mathbb{P}(X \le 2) \approx e^{-3}(1+3+4.5) \approx 0.423.$$

**Не уместен:** $n = 20$, $p = 0.4$ → $\lambda = 8$, но $p = 0.4$ велико. Используем $\mathrm{Bin}(20, 0.4)$.

Правило: Пуассон хорош при $n \ge 20$, $p \le 0.05$ и $\lambda = np \le 10$.
